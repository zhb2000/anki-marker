...
- 新增 macOS“关闭窗口行为”设置：可选择关闭窗口后应用是否**保持后台运行**（默认保持，关闭后仅隐藏窗口，可由 Dock 图标、菜单栏图标或全局快捷键再次唤起；关闭此项则点关闭按钮直接退出应用）；保持运行时可进一步选择后台运行期间应用图标的显示位置——**菜单栏**（屏幕右上角，默认）、**Dock 栏**或**不显示图标**。窗口打开时图标始终显示在 Dock 栏，仅在后台运行期间按此选项显示。
- 后台运行期间，菜单栏图标和 Dock 栏图标的右键菜单均新增**划词录入**入口：在任意应用中选中文字后点击菜单项即可录入划词面板，与全局快捷键等效；若已设置全局快捷键，菜单项中会显示快捷键提示。不设置快捷键的用户现在也可以仅通过鼠标完成划词录入。
